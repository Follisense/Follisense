    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: dm, padding: '40px 20px 80px' }}>
      ...
      <div className="adm">
        ...
        <div className="adm-grid" style={{ marginBottom: 12 }}>
          ...
          <Metric label="Came back" value={data.returning_users} sub={`${pct(data.returning_users, data.onboarded_users)}% of onboarded`} />
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px', marginBottom: 12 }}>
          ...
          <p style={{ fontSize: 15, color: C.ink, margin: 0, lineHeight: 1.6 }}>
            {data.returning_users} of {data.onboarded_users} people who finished onboarding have done a second check-in.
            {data.onboarded_users > 0 && ` That is ${pct(data.returning_users, data.onboarded_users)}%.`}
          </p>
        </div>

        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px', marginBottom: 12 }}>
          <p style={{ fontSize: 11, color: C.muted, margin: '0 0 12px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Signups, last 14 days</p>
        ...
        {triageTotal > 0 && (
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px', marginBottom: 12 }}>
            <p style={{ fontSize: 11, color: C.muted, margin: '0 0 12px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Triage split</p>
              ...
              <tbody>
                {data.users.map(u => (
                  <tr key={u.id} style={{ borderTop: `1px solid ${C.border}`, color: C.ink }}>
                    <td style={{ padding: '9px 0', fontFamily: 'monospace', fontSize: 12 }}>{u.id}</td>
